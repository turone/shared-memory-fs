...
| `mkdir` / `rm` / `rename`                    | void \| Promise                                       | `mkdir` is a no-op; `ENOTSUP` for `appendFile` / moving a prepared key and for a virtual directory `rename` |
...
- A directory renamed across a place's boundary — into, out of or between
  ...
  at once. And any directory rename in a virtual place: its directories
  are implicit, only entries move.
...
- A directory moves natively within one disk-origin place — the watcher
  ...
  places is `ENOTSUP`, and so is one in a virtual place, whose directories
  are implicit. `disk` and `node-default` places are plain `node:fs`
  territory.
