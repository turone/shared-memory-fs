...
class VFSKernel {
  ...
  resolveFsPath(filePath) {
    if (this.sealed && this.sealIndex) {
      const abs = path.resolve(filePath);
      return this.sealIndex.get(abs) || null;
    }
    const place = this.registry.resolve('fs', filePath);
    this.policy.logRoute('fs', 'fs', filePath, place);
    if (!place) return null;
    const abs = path.resolve(filePath);
    const rel = path.relative(this.appRoot, abs);
    const key = '/' + rel.replace(/\\/g, '/');
    const dirPrefix = place.config.match.dir;
    const fileKey = dirPrefix ? key.substring(dirPrefix.length + 1) : key;
    return { place, key, fileKey };
  }
  ...
  seal() {
    ...
    for (const place of this.registry.getPlaces()) {
      ...
      for (const fk of place.files.keys()) {
        this.#addToSealIndex(place, fk, dirPrefix);
      }
    }
    this.console.debug(`[vfs] sealed, ${this.sealIndex.size} entries indexed`);
  }

  #addToSealIndex(place, fileKey, dirPrefix) {
    ...
    this.sealIndex.set(abs, { place, key: appKey, fileKey });
  }

  #removeFromSealIndex(place, fileKey) {
    const dirPrefix = place.config.match.dir || '';
    const appKey = dirPrefix ? '/' + dirPrefix + fileKey : fileKey;
    const relPath = appKey.startsWith('/') ? appKey.substring(1) : appKey;
    const abs = path.resolve(this.appRoot, relPath);
    this.sealIndex.delete(abs);
  }
  ...
  #handleUpdate(data) {
    ...
    for (const [key, entry] of updates) {
      ...
      if (this.sealed && this.sealIndex && place) {
        this.#addToSealIndex(place, key);
      }
    }
  }
  ...
  #handleDelete(data) {
    ...
    for (const key of keys) {
      files.delete(key);
      if (this.sealed && this.sealIndex && place) {
        this.#removeFromSealIndex(place, key);
      }
    }
  }
