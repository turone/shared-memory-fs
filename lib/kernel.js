...
const { ModuleCache, cacheKeyOf } = require('./module-cache.js');
const { CompressionCache } = require('./compression-cache.js');
const { isCompanionKey, companionKey } = require('./companion.js');
const { PlacementRegistry, absPathOf } = require('./registry.js');
...
// assetKey must equal mount or start with mount+'/'; the rest becomes the
// file key (with leading '/'). Returns null on no match.
...
// Segments must fit the most permissive place, otherwise its large files
// could never be allocated; each place still caps its own files.
const maxFileSizeOf = (config) => {
  let max = config.global.memory.maxFileSize;
  for (const place of config.places) {
    if (place.maxFileSize > max) max = place.maxFileSize;
  }
  return max;
};
...
class VFSKernel {
  ...
  async initialize() {
    ...
    this.cache = new FilesystemCache({
      ...
      maxFileSize: maxFileSizeOf(this.config),
      reader,
    });
    ...
    this.moduleCache = new ModuleCache({
      ...
      projectInto: (mount, key, entry, files) =>
        this.#projectInto(mount, key, entry, files),
    });
  ...
  #storePredicate(place) {
    ...
    if (compress.retainRaw || compress.codecs.length === 0) return null;
  ...
  // assets. Asset keys whose prefix matches the place's `dir` are mapped to
  // Place file keys '/<rest>'. Assets outside the
  ...
  #memoryWrite(place, key, data) {
    ...
    if (place.config.compile && fileExt(key) === 'js') {
      const bytecode = this.moduleCache.createBytecode(
        buf.toString('utf8'),
        key,
      );
      if (bytecode) {
  ...
  #memoryDelete(place, key) {
    ...
    place.files.delete(cacheKeyOf(key));
  ...
  async #compileAll() {
    for (const pc of this.config.places) {
      ...
      if (!place) continue;
      await this.moduleCache.compilePlace(pc.dir, place);
    }
  }
  ...
  async #compressAll() {
    for (const pc of this.config.places) {
      if (pc.compress.codecs.length === 0) continue;
      const place = this.registry.getPlace(pc.name);
      if (!place) continue;
      const readSource = (key) => this.#readSource(place, key);
      await this.compressionCache.compressPlace(pc.dir, place, readSource);
    }
  }

  // Source bytes for compression: a SAB view when raw is retained, otherwise
  // a read of the file the disk entry points at.
  #readSource(place, key) {
    const file = place.files.get(key);
    if (!file) return null;
    if (file.data) return file.data;
    if (!file.path) return null;
    return fsp.readFile(file.path).catch((err) => {
      this.console.warn(
  ...
  #addToPathIndex(place, fileKey) {
    if (isCompanionKey(fileKey)) return;
    const abs = absPathOf(this.appRoot, place, fileKey);
    const appKey = '/' + place.mount + fileKey;
  ...
  async #processChange(ep, route, filePath) {
    ...
    if (stat.isDirectory()) {
      ...
      for (const [key] of subFiles) {
        ...
        await this.#recompress(ep, mount, key, newEntry, place);
      }
      return;
    }
    ...
    if (
      this.moduleCache.isCompilable(place, key) &&
      newEntry.kind === 'shared'
    ) {
      const r = await this.moduleCache.compileFromEntry(mount, key, newEntry);
      if (r) {
        this.#stageUpdate(ep, mount, r.cacheKey, r.cacheEntry, place);
        if (r.oldCache && r.oldCache.kind === 'shared')
          ep.oldEntries.push(r.oldCache);
      }
    }
  ...
  // Rebuild every representation of a changed source in the current epoch.
  // A codec that fails now must not leave its previous companion behind:
  // it still holds the old content and would be served alongside the new
  // source, so it is staged for removal in the very same message.
  async #recompress(ep, mount, key, newEntry, place) {
    if (!this.compressionCache.compressible(place, key)) return;
    const srcBuf = await this.#readEntrySource(mount, key, newEntry, place);
    if (!srcBuf) return;
    const stat = newEntry.stat;
    const { built, failed } = await this.compressionCache.compressBuffer(
      mount,
      key,
      srcBuf,
      stat,
    ...
    for (const { key: cacheKey, entry, oldEntry } of built) {
      this.#stageUpdate(ep, mount, cacheKey, entry, place);
      if (oldEntry && oldEntry.kind === 'shared') ep.oldEntries.push(oldEntry);
    }
    for (const encoding of failed) {
      const cacheKey = companionKey(key, encoding);
      const stale = this.cache.remove(mount, cacheKey);
      if (!stale) continue;
      this.#stageRemoval(ep, mount, cacheKey, place);
      if (stale.kind === 'shared') ep.oldEntries.push(stale);
    }
  }

  // Source bytes of a just-allocated entry: a SAB view, or a disk read when
  // the place keeps only compressed representations.
  async #readEntrySource(mount, key, entry, place) {
    if (entry.kind === 'shared') {
      if (entry.length === 0) return Buffer.alloc(0);
      const seg = this.cache.getSegment(entry.segmentId);
      return Buffer.from(seg.sab, entry.offset, entry.length);
    }
    if (!entry.path) return null;
    try {
      return await fsp.readFile(entry.path);
    } catch (err) {
      this.console.warn(
        `[vfs] place "${place.name}": cannot read "${key}" — ${err.message}`,
      );
  ...
  #stageUpdate(ep, mount, key, entry, place) {
    const group = this.#group(ep, mount, place);
  ...
  // Removals travel inside the same file-update message as the new source:
  // a separate delete message would leave a window where a worker sees the
  // new source next to a stale representation.
  #stageRemoval(ep, mount, key, place) {
    this.#group(ep, mount, place).removals.push(key);
  }

  #group(ep, mount, place) {
    const group = ep.updates[mount];
    if (group) return group;
    return (ep.updates[mount] = {
      entries: [],
      removals: [],
      segmentIds: new Set(),
      place,
  ...
  #processDelete(ep, route, filePath) {
    ...
    for (const key of keys) {
      ...
      for (const cacheKey of this.#companionsOf(place, key)) {
        const stale = this.cache.remove(mount, cacheKey);
        if (!stale) continue;
  ...
  *#companionsOf(place, key) {
    if (place.config.compile) yield cacheKeyOf(key);
    for (const { encoding } of place.config.compress.codecs) {
      yield companionKey(key, encoding);
    }
  }
  ...
  #flushEpoch(epoch) {
    ...
    for (const mount of Object.keys(updates)) {
      const { entries, removals, segmentIds } = updates[mount];
  ...
  #applyUpdate(data) {
    const { target, updates, removals, newSegments } = data;
    if (newSegments) {
