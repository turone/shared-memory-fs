...
// `fs.script` nests inside the fs domain: the sources a preparer turns into
// canonical content and the library compiles into `vm.Script` cached data.
...
// `fs.script` — sources the library treats as JavaScript to publish and,
// optionally, to compile. `prepare` names a preparer (`VfsKernel` option
// `preparers`) that turns the raw file into the canonical VFS content;
// `compile` builds `vm.Script` cached data for `PlaceFs.script()`.
const resolveScript = (where, raw) => {
  if (raw === undefined || raw === false) return null;
  if (raw === true) raw = {};
  if (!isObject(raw)) fail(`${where} must be true, false or an object`);
  knownKeys(where, raw, ['ext', 'prepare', 'compile']);
  const { prepare } = raw;
  if (prepare !== undefined && prepare !== null) {
    if (typeof prepare !== 'string' || !PREPARER_RE.test(prepare)) {
      fail(
        `${where}.prepare must be a preparer identifier ` +
          '(a name from the kernel option `preparers`), got ' +
          (typeof prepare === 'function' ? 'a function' : String(prepare)),
      );
    }
  }
  return {
...
const resolveFs = (where, raw) => {
  ...
  if (raw === true) raw = {};
  if (!isObject(raw)) fail(`${where} must be true, false or an object`);
  knownKeys(where, raw, ['ext', 'writable', 'zeroCopy', 'compress', 'script']);
...
// The script domain: sources consumers compile themselves with `vm.Script`
// through `PlaceFs.script()`. Every visible source gets V8 cached data;
// `prepare` names a preparer (`VfsKernel` option `preparers`) that turns
// the raw file into the source the VFS publishes as canonical content.
