...
class MapStore {
  ...
  // its mtime, like a rename on disk. A directory is implicit, its entries
  // are what moves: renaming one is unsupported.
  rename(from, to) {
    const { place } = this;
    ...
    if (!current && place.isDirectory(from)) throw fail('ENOTSUP', 'directory');
