...
// FilesystemCache — pooled SharedArrayBuffer file cache. Zero Node API
// dependencies: file contents come either from `file.data` (Buffer) or the
// injected `options.reader(path, sab, offset, size)` that reads the file
// directly into the SharedArrayBuffer.
//
// Data contract:
//
// @typedef {Object} FileInput             // value in `filesMap` / `allocate`
// @property {Buffer} [data]               // in-memory content, or
// @property {string} [path]               // path passed to injected reader
// @property {{size:number}} [stat]        // size hint (sorting, stats)
//
...
// @property {{size:number}} stat
//
// @typedef {Object} DiskEntry             // file too large for the cache
// @property {'disk'} kind
// @property {string|null} path
// @property {null} data
// @property {{size:number}} stat
//
// @typedef {Object} FsIndex               // one namespace (mount)
...
class FilesystemCache {
  ...
  // a disk entry (used when a place keeps only compressed representations).
  // `options.maxFileSize` overrides the cache-wide limit for this call.
  async load(name, filesMap, options = {}) {
    const { store, maxFileSize } = options;
    ...
    for (const [key, file] of candidates) {
      const entry =
        store && !store(key)
          ? diskEntry(file)
          : await this.#allocateEntry(file, segmentIds, maxFileSize);
  ...
  // `options.fallback === false` returns null and registers nothing when the
  // data cannot be placed in SAB. Callers whose bytes exist only in memory
  // (compressed representations) must not get a disk entry pointing at the
  // raw file on disk.
  async allocate(name, key, file, options = {}) {
    ...
    const { store, fallback, maxFileSize } = options;
    const entry =
      store && !store(key)
        ? diskEntry(file)
        : await this.#allocateEntry(file, index.segmentIds, maxFileSize);
    if (entry.kind === 'disk' && fallback === false) return null;
