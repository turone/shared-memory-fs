...
class FilesystemCache {
  ...
  // In-memory inputs (SEA assets, prepared sources) have no disk fallback:
  // one that cannot be placed fails the load.
  async load(name, files, options = {}) {
    ...
    for (const [key, file] of sorted) {
      ...
      if (entry.kind === 'disk' && !entry.path) {
        throw new Error(`"${key}" does not fit in SAB and has no disk copy`);
      }
  ...
  // data exists only in memory (bytecode, compressed representations,
  // prepared sources) must never get a disk entry pointing at the raw file.
  ...
  // Undo a provisional allocation: the previous entry (or none) is
  // reinstated and the new bytes return to the pool at once. Safe only
  // while the new entry has not been published — no worker projects it.
  rollback(name, key, entry, oldEntry) {
    const index = this.index(name);
    if (oldEntry) index.entries.set(key, oldEntry);
