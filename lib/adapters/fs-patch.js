...
//   directory renamed into or out of a place;
...
const copyOptions = (syscall, arg) => {
  const cp = syscall === 'cp' ? arg || {} : {};
  const mode = (syscall === 'cp' ? cp.mode : arg) || 0;
  let unsupported = null;
  if (mode & COPYFILE_FICLONE_FORCE) unsupported = 'COPYFILE_FICLONE_FORCE';
  else if (cp.filter) unsupported = 'filter';
  else if (cp.preserveTimestamps) unsupported = 'preserveTimestamps';
  ...
  if (syscall === 'copyfile' && mode & COPYFILE_EXCL) existing = 'EEXIST';
  else if (cp.force === false) {
    existing = cp.errorOnExist ? 'ERR_FS_CP_EEXIST' : 'skip';
  }
  return { unsupported, existing, links: syscall === 'cp' && !cp.dereference };
};
...
const copyThrough = (source, target, { from, to, options, sync, fail }) => {
  ...
  if (sync && store && target.place.store?.sync === false) {
  ...
  if (flag === 'wx') {
    ...
    if (exists) throw Object.assign(fail('EEXIST'), { code: options.existing });
  }
  const write = (bytes) => {
    if (store) return facadeOf(target).writeFile(target.key, bytes);
    return sync
      ? disk.writeFile(to, bytes, { flag })
      : disk.writeFileAsync(to, bytes, { flag });
  };
  if (source.kind === 'canonical') {
    return write(facadeOf(source).readFile(source.key));
  }
  if (sync) return write(disk.readFile(from));
  return disk.readFileAsync(from).then(write);
};
