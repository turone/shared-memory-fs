...
// A write into the destination — its store or the disk — that fails, fails
// as the copy: its code under the copy's syscall and paths, the write's own
// error as the cause. Other errors (a preparer's) pass as they are.
const asCopy = (fail, write) => {
  const refuse = (err) => {
...
const copyThrough = (source, target, { from, to, options, sync, fail }) => {
  ...
  const write = (bytes) =>
    asCopy(fail, () =>
      store
        ? facadeOf(target).writeFile(target.key, bytes)
