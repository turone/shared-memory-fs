...
class SabStore {
  ...
  // pipeline, prepared when the new extension has a preparer. A directory
  // is implicit, its entries are what moves: renaming one is unsupported.
  rename(from, to) {
    return this.#run([from, to], () => {
      ...
      if (!found && place.isDirectory(from)) throw fail('ENOTSUP', 'directory');
